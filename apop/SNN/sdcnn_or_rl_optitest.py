...
sys.path.append(os.path.abspath(__file__).replace('SNN/sdcnn_or_rl_optitest.py', ''))
...
class CustomDataset(torch.utils.data.Dataset):
  def __init__(self, save_folder='_tmp_layer2_output_folder/'):
    self.folder = save_folder
    # self.inputs_spikes = h5py.File(os.path.join(save_folder, 'all_outputs.h5'), 'r')
    self.all_targets = torch.load(os.path.join(save_folder, 'all_targets.pt'))
  
  def __len__(self):
    return len(self.all_targets)
  
  def __getitem__(self, i):
    with h5py.File(os.path.join(self.folder, 'all_outputs.h5'), 'r') as hf:
      input_spikes = hf[str(i)][()]
    return torch.FloatTensor(input_spikes), self.all_targets[i]
...
class DataTransformer(object):
  ...
  def __call__(self, image):
    ...
    # image = DataTransformer.norm_between_zero_one(image)
    return toci.cumulative_intensity_to_latency(image, self.config['n_time_steps'])
  
  @staticmethod
  def norm_between_zero_one(tensor):
    ori_shape = tensor.shape
    tensor = tensor.view(tensor.size(1), -1)
    tensor -= tensor.min(1, keepdim=True)[0]
    tensor /= tensor.max(1, keepdim=True)[0]
    return tensor.view(ori_shape)


class DataTransformerHumanColor(object):
  def __init__(self, configuration={}):
    base_config = {'kernel_type': toci.DoGKernel, 'kernels_conf': [[7, 7/9, 14/9], [7, 14/9, 7/9],
                                                                   [13, 13/9, 26/9], [13, 26/9, 13/9]],
                   'padding': 3, 'threshold': 50, 'use_threshold': True, 'normalization_radius': 8, 'n_time_steps': 20}
    self.config = {**base_config, **configuration}

    self.to_tensor = torchvision.transforms.ToTensor()
    self.filters = toci.Filter(self.config)
  
  def get_color_view(self, pos_idx, neg_idx, red_pos_neg, green_pos_neg, blue_pos_neg, yellow_pos_neg):
    red_pos_green_neg = red_pos_neg[0, pos_idx] - green_pos_neg[0, neg_idx]  # [28, 28]
    green_pos_red_neg = green_pos_neg[0, pos_idx] - red_pos_neg[0, neg_idx]
    blue_pos_yellow_neg = blue_pos_neg[0, pos_idx] - yellow_pos_neg[0, neg_idx]
    return torch.stack([red_pos_green_neg, green_pos_red_neg, blue_pos_yellow_neg]).unsqueeze(0)
  
  def __call__(self, image):
    image = self.to_tensor(image.convert('RGB')) * 255  # [3, 28, 28]

    red_pos_neg = self.filters(image[0].unsqueeze(0).unsqueeze(0) * 255)
    green_pos_neg = self.filters(image[1].unsqueeze(0).unsqueeze(0) * 255)
    blue_pos_neg = self.filters(image[2].unsqueeze(0).unsqueeze(0) * 255)
    yellow_pos_neg = self.filters((image[0] + image[1]).unsqueeze(0).unsqueeze(0) * 255)

    # image = self.get_color_view(0, 1, red_pos_neg, green_pos_neg, blue_pos_neg, yellow_pos_neg)
    image7 = self.get_color_view(0, 1, red_pos_neg, green_pos_neg, blue_pos_neg, yellow_pos_neg)
    image13 = self.get_color_view(2, 3, red_pos_neg, green_pos_neg, blue_pos_neg, yellow_pos_neg)
    image = torch.cat([image7, image13], 1)
    # image = DataTransformer.norm_between_zero_one(image)
    # image = toci.local_normalization(image, self.config['normalization_radius'])
    return toci.cumulative_intensity_to_latency(image, self.config['n_time_steps'])
...
class SDCNNExperiment(object):
  def __init__(self, configuration={}):
    self.base_config = {'layers_config': [{'in_channels': 6, 'out_channels': 30, 'kernel_size': 5, 'stride': 1,
                                           'weight_mean': 0.8, 'weight_std': 0.05, 'anti_stdp': False},
                                          # {'in_channels': 30, 'out_channels': 30, 'kernel_size': 5, 'stride': 2, 'anti_stdp': False},
                                          {'in_channels': 30, 'out_channels': 250, 'kernel_size': 3, 'stride': 1,
                                           'weight_mean': 0.8, 'weight_std': 0.05, 'anti_stdp': False},
                                          # {'in_channels': 250, 'out_channels': 250, 'kernel_size': 3, 'stride': 2, 'anti_stdp': False},
                        ...
                        'anti_learning_rates': (-0.004, 0.0005), 'coef_evo_lr': 2,
                        ...
                        'n_winners': [5, 8, 1], 'inhibition_radius': [3, 1, 0], 'strategy': 'wta',
  ...
  def update_output_size(self):
    if self.layers[-1].weights.size(0) < 160:
      # DOUBLE EACH TIME
      # new_weights = torch.FloatTensor(self.layers[-1].weights.shape).normal_(0.8, 0.02).to(self.device)
      # self.layers[-1].weights = torch.nn.Parameter(torch.cat([self.layers[-1].weights, new_weights], dim=0), requires_grad=False)
      # self.decision_map += self.decision_map
      # self.learning_rates[-1] += self.learning_rates[-1]
      # self.anti_learning_rates[-1] += self.anti_learning_rates[-1]
      # UPDATE 10 by 10
      new_weights = torch.FloatTensor(10, 250, 5, 5).normal_(0.8, 0.02).to(self.device)
      self.layers[-1].weights = torch.nn.Parameter(torch.cat([self.layers[-1].weights, new_weights], dim=0), requires_grad=False)
      self.decision_map += self.decision_map[:10]
      self.learning_rates[-1] += self.learning_rates[-1][:10]
      self.anti_learning_rates[-1] += self.anti_learning_rates[-1][:10]
  ...
  def test_update_rstdp_lr(self, layer_idx):
    new_ap = torch.min(self.learning_rates[layer_idx][0][0] * 2, torch.tensor([0.01]).cuda())#self.config['max_ap'])
    new_an = self.learning_rates[layer_idx][0][0] * self.config['an_update']
    self.update_all_lr(new_ap, new_an, layer_idx)
    self.update_all_anti_lr(new_an, new_ap, layer_idx)
  ...
  def competition_winner_stdp(self, input_spikes, potentials, layer_idx, target, stdp=True):
    ...
    # logging.info(f'n_winners = {len(winners)}')
    ...
    if stdp:
      winners_shifted = None
      if self.layers[layer_idx].stride == 2:
        winners_shifted = [[w[0], w[1] * 2, w[2] * 2] for w in winners]
      lf.functional_stdp(self.layers[layer_idx], lrs, input_spikes, spikes, winners, strategy=self.config['strategy'],
                         use_stabilizer=use_stabilizer, lower_bound=lower_bound, upper_bound=upper_bound,
                         winners_shifted=winners_shifted)
    return spikes, potentials
  
  def forward(self, input_spikes, layer_idx, target=None, training=False, ignore_layers=[], epoch_iter='0_0'):
    for i, layer in enumerate(self.layers):
      ...
      if i == layer_idx and training:
        # self.store_stats.create_dataset(epoch_iter, data=spikes.cpu().count_nonzero(dim=(1, 2, 3)).tolist(),
        #                                 compression='gzip', compression_opts=9)
        # self.store_stats.create_dataset(epoch_iter, data=(spikes.cpu() != 0).sum((1, 2, 3)).tolist(),
        #                                 compression='gzip', compression_opts=9)
  ...
  def train(self):
    for i, n_epoch in enumerate(tqdm(self.config['n_epochs'])):
      # self.store_stats = h5py.File(f'_tmp_nonzero_elmt_timestep_along_training_layer_{i}.h5', 'w')
      for epoch in tqdm(range(n_epoch), leave=False):
        # if i == 2 and epoch == 0:
        #   self.write_stdp_layer2_output_h5()
        #   self.train_data_loader = torch.utils.data.DataLoader(CustomDataset(), batch_size=self.config['batch_size'],
        ...
        for j, (spikes_in, target) in enumerate(tqdm(self.train_data_loader, leave=False)):
          if i < len(self.layers) - 1 and j > 0 and j % self.config['timestep_update_lr'] == 0:
            new_ap = torch.min(self.learning_rates[i][0][0] * self.config['coef_evo_lr'], self.config['max_ap'])
            new_an = self.learning_rates[i][0][0] * self.config['an_update']
            self.update_all_lr(new_ap, new_an, i)
            # logging.info(f'n_neuron_non_converged = {((self.layers[i].weights * (1 - self.layers[i].weights)).sum((1, 2, 3)).int() != 0).sum()}')
            # logging.info(f'n_neuron_learner = {self.layers[i].weights.sum((1,2,3)).int().tolist()}')
            # logging.info(f'convergence = {(self.layers[i].weights * (1 - self.layers[i].weights)).sum((1, 2, 3)).int().tolist()}')
          
          self.forward(spikes_in.to(self.device), i, target=target if i == len(self.layers) - 1 else None, training=True,
                       epoch_iter=f'{epoch}_{j}')
          # self.forward(spikes_in.to(self.device), i, target=target if i == 2 else None, training=True,
          #              ignore_layers=[] if i < 2 else [0, 1])

          if i == len(self.layers) - 1 and self.config['adaptive_lr'] and j > 0 and j % 10000 == 0:
            # self.test_update_rstdp_lr(i)
          ...
          if j % self.config['check_convergence_step'] == 0:
            ...
            if (C < self.config['convergence_threshold'] and epoch != 0) or round(C.item(), 4) == 0.:
              self.save_model()
              break
      # self.store_stats.close()
  ...
  def dump_output_potentials_targets_preds(self, train=False, save_name='_tmp_output_potentials_targets_preds.pt'):
    targets, predictions, potentials = [], [], []
    for spikes_in, target in tqdm(self.train_data_loader if train else self.test_data_loader):
      pots, _, pred = self.forward(spikes_in.to(self.device), len(self.layers) - 1, target=target)
      targets.append(target)
      predictions.append(pred)
      potentials.append(pots.cpu().short())
    
    torch.save({'targets': targets, 'predictions': predictions, 'potentials': potentials}, save_name)
  ...
  def write_wds_from_stdp_layers(self, save_name='sdcnn_layer1-2_output.tar'):
    sink = wds.TarWriter(save_name)
    for index, (input_spikes, target) in enumerate(tqdm(self.train_data_loader)):
      input_final_layer = self.forward(input_spikes.to(self.device), None).cpu()
      if index%1000==0:
        print(f"{index:6d}", end="\r", flush=True, file=sys.stderr)
      sink.write({
        "__key__": "sample%06d" % index,
        "input_final_layer.pyd": input_final_layer,
        "target.pyd": target,
      })
    sink.close()
  
  def write_stdp_layer2_output(self, save_folder='_tmp_layer2_output_folder/', save_name='output_layer2_{}'):
    if not os.path.isdir(save_folder):
      os.makedirs(save_folder)

    for i, (input_spikes, target) in enumerate(tqdm(self.train_data_loader)):
      input_final_layer = self.forward(input_spikes.to(self.device), None).cpu()
      torch.save([input_final_layer, target], os.path.join(save_folder, save_name.format(i)))
  
  def write_stdp_layer2_output_h5(self, save_folder='_tmp_layer2_output_folder/', force_rewrite=False):
    if not os.path.isdir(save_folder):
      os.makedirs(save_folder)
    
    if force_rewrite or not os.path.isfile(os.path.join(save_folder, 'all_outputs.h5'))\
                     or not os.path.isfile(os.path.join(save_folder, 'all_targets.pt')):
      targets = []
      with h5py.File(os.path.join(save_folder, 'all_outputs.h5'), 'w') as hf:
        for i, (input_spikes, target) in enumerate(tqdm(self.train_data_loader)):
          input_final_layer = self.forward(input_spikes.to(self.device), None).cpu().numpy()
          hf.create_dataset(str(i), data=input_final_layer, compression='gzip', compression_opts=1)
          targets.append(target)
      
      torch.save(targets, os.path.join(save_folder, 'all_targets.pt'))


if __name__ == "__main__":
  # Layer 1 is trained in 2min (STDP)
  # Layer 2 is trained in 8min (STDP)
  # Layer 3 achieve performance of 0.9 in 22min
  # if we set n_time_steps = 20 (instead of 15) the layer 3 achieve 0.9 in 10min
  # 35min for 0.93
  argparser = argparse.ArgumentParser(prog='sdcnn_or_rl_optitest.py', description='')
  ...
  argparser.add_argument('--save_dump', default='_tmp_output_potentials_targets_preds.pt', type=str)
  argparser.add_argument('--train', default=False, type=ast.literal_eval)
  argparser.add_argument('--save_model', default='model/sdcnn_rl_exp.pt', type=str)
  ...
  if rep == 'y':
    print(f'Loading model {args.save_model}')
    sdcnn_rl_exp.load_model(save_name=args.save_model)
  ...
  rep = input('Evaluate STDP layer with linearSVC? (y or n): ')
  if rep == 'y':
    sdcnn_rl_exp.evaluate_stdp_layers()
  
  rep = input('Dump output potentials-preds-targets? (y or n): ')
  if rep == 'y':
    sdcnn_rl_exp.dump_output_potentials_targets_preds(train=args.train, save_name=args.save_dump)