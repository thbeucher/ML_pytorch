...
class ACCDCGANTrainer(GANTrainer):
  BASE_CONFIG = {'save_name': 'model/mnist_accdcgan_model.pt', 'save_img_folder': 'generated_accdcgan_imgs/',
                 'betas': (0.5, 0.999)}
  def __init__(self, config):
    super().__init__({**ACCDCGANTrainer.BASE_CONFIG, **config})
  ...
  def instanciate_model(self):
    self.discriminator = m.ACCNNDiscriminator({}).to(self.device)
    self.generator = m.ConditionalCNNGenerator({'merging_type': 'mul'}).to(self.device)
