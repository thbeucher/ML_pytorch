...
class PositionalEmbedder(nn.Module):
  ...
  '''
  def __init__(self, max_seq_len, embedding_size, d_model, output_size=0, reduce_dim=False, scaling=True, dropout=0.1,
               kernel=3, stride=2):
    '''
    Params:
      * max_seq_len : int
    ...
    self.scale = torch.sqrt(torch.FloatTensor([d_model])) if self.scaling else 1
