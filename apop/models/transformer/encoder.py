...
class GatedEncoderBlock(nn.Module):
  ...
  def __init__(self, d_model, d_keys, d_values, n_heads, d_ff, dropout=0.1, act_fn='relu', mode='output'):
    ...
    if mode == 'gru':
      ...
      self.gate = self._gated_highway if mode == 'highway' else self._gated_output

    self.dropout = nn.Dropout(dropout)
  
  def _gated_gru(self, x, y, n=0):
  ...
  def _gated_output(self, x, y, n=0):
