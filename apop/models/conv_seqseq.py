...
class EncoderEmbedder(nn.Module):
  def __init__(self, input_dim, emb_dim, hid_dim, max_seq_len, dropout, reduce_dim=False, kernel=3, stride=2):
    super().__init__()
  ...
  def forward(self, x):
    ...
    index_x = torch.LongTensor(range(seq_len)).unsqueeze(0).repeat(batch_size, 1).to(x.device)
...
class DecoderEmbedder(nn.Module):
  def __init__(self, input_dim, emb_dim, max_seq_len, dropout):
    super().__init__()
  ...
  def forward(self, x):
    ...
    pos = torch.arange(0, x.shape[1]).unsqueeze(0).repeat(x.shape[0], 1).to(x.device)
...
class LMEncoderEmbedder(nn.Module):
  def __init__(self, input_dim, emb_dim, max_seq_len):
    super().__init__()
...
class Encoder(nn.Module):
  def __init__(self, emb_dim, hid_dim, n_layers, kernel_size, dropout, embedder=None, max_seq_len=100, input_dim=None):
    super().__init__()
    ...
    self.encoders = nn.ModuleList([EncoderBlock(hid_dim, kernel_size, dropout) for _ in range(n_layers)])
    self.hid2emb = nn.Linear(hid_dim, emb_dim)

    self.embedder = EncoderEmbedder(input_dim, emb_dim, hid_dim, max_seq_len, dropout) if embedder is None else embedder
  ...
  def forward(self, x):
    ...
    combined = (conved + embedded) * self.scale.to(conved.device)
...
class EncoderBlock(nn.Module):
  ...
  def forward(self, x):
    ...
    conved = (conved + x) * self.scale.to(x.device)  # residual connection
    return conved
...
class Decoder(nn.Module):
  def __init__(self, output_dim, emb_dim, hid_dim, n_layers, kernel_size, dropout, pad_idx, embedder=None, max_seq_len=100,
    ...
    self.embedder = DecoderEmbedder(output_dim, emb_dim, max_seq_len, dropout) if embedder is None else embedder

    self.emb2hid = nn.Linear(emb_dim, hid_dim)
    self.decoders = nn.ModuleList([DecoderBlock(hid_dim, emb_dim, kernel_size, pad_idx, dropout, score_fn=score_fn,
...
class DecoderBlock(nn.Module):
  def __init__(self, hid_dim, emb_dim, kernel_size, pad_idx, dropout, score_fn=F.softmax, scaling_energy=False,
    ...
    self.attention = Attention(hid_dim, emb_dim, score_fn=score_fn, scaling_energy=scaling_energy,
...
class Attention(nn.Module):
  def __init__(self, hid_dim, emb_dim, score_fn=F.softmax, scaling_energy=False, multi_head=False, d_keys_values=64, dropout=0.):
    super().__init__()
  ...
  def forward(self, embedded, conved, encoder_conved, encoder_combined):
    ...
    if self.multi_head:
      ...
      energy = combined.matmul(encoder_conved.permute(0, 2, 1)) / self.scaling_energy.to(combined.device)
...
class Seq2Seq(nn.Module):
  ...
  def beam_search_decoding(self, enc_in, sos_idx, eos_idx, pad_idx, max_seq_len=600, beam_size=5, alpha=0.7):
    ...
    batch_size = enc_in.shape[0]
    output_seq = []
    sentence_lengths = torch.zeros(batch_size, beam_size).to(encoder_conved.device)
    beam_scores = torch.zeros(batch_size * beam_size, 1).to(encoder_conved.device)
    finished = torch.zeros(batch_size, beam_size, dtype=torch.uint8).to(encoder_conved.device)
    
    dec_in = torch.LongTensor(batch_size, 1).fill_(sos_idx).to(encoder_conved.device)
    for_rows = torch.LongTensor([i * beam_size for i in range(batch_size)]).unsqueeze(1).repeat(1, beam_size).to(encoder_conved.device)
...
class AudioTextLM(nn.Module):
  def __init__(self, audio_encoder, audio_decoder, lm_encoder, lm_decoder):
    super().__init__()
...
class DecoderRelu(nn.Module):
  def __init__(self, output_dim, emb_dim, hid_dim, n_layers, kernel_size, dropout, pad_idx, embedder=None, max_seq_len=100,
    ...
    self.emb2hid = nn.Linear(emb_dim, hid_dim)
    self.decoders = nn.ModuleList([DecoderBlockRelu(hid_dim, emb_dim, kernel_size, pad_idx, dropout, score_fn=score_fn,
                                                    scaling_energy=scaling_energy, multi_head=multi_head, d_keys_values=d_keys_values)
                                                      for _ in range(n_layers)])
    self.hid2emb = nn.Linear(hid_dim, emb_dim)
...
class DecoderBlockRelu(nn.Module):
  def __init__(self, hid_dim, emb_dim, kernel_size, pad_idx, dropout, score_fn=F.softmax, scaling_energy=False,
    ...
    self.attention = AttentionRelu(hid_dim, emb_dim, score_fn=score_fn, scaling_energy=scaling_energy,
                                   multi_head=multi_head, d_keys_values=d_keys_values, dropout=dropout)
...
class EncoderMixKernelRelu(nn.Module):
  def __init__(self, emb_dim, hid_dim, n_layers, kernel_sizes, dropout, embedder=None, max_seq_len=100, input_dim=None):
    '''
    Params:
    ...
    self.encoders = nn.ModuleList(
                      [
                        nn.Sequential(
                          OrderedDict(
                            [(f'encoder{i}_{j}', EncoderBlock(hid_dim, ker, dropout)) for j in range(n_layers[i])]
                          )
                        )
...
class DecoderFeedback(nn.Module):
  def __init__(self, output_dim, emb_dim, hid_dim, n_layers, kernel_size, dropout, pad_idx, embedder=None, max_seq_len=100,
    ...
    self.emb2hid = nn.Linear(emb_dim, hid_dim)
    self.decoders = nn.ModuleList([DecoderBlockFeedback(hid_dim, emb_dim, kernel_size, pad_idx, dropout, score_fn=score_fn,
                                                        scaling_energy=scaling_energy, multi_head=multi_head, d_keys_values=d_keys_values)
                                      for _ in range(n_layers)])
    self.hid2emb = nn.Linear(hid_dim, emb_dim)
...
class DecoderBlockFeedback(nn.Module):
  def __init__(self, hid_dim, emb_dim, kernel_size, pad_idx, dropout, score_fn=F.softmax, scaling_energy=False,
    ...
    self.attention = AttentionFeedback(hid_dim, emb_dim, score_fn=score_fn, scaling_energy=scaling_energy,
                                       multi_head=multi_head, d_keys_values=d_keys_values, dropout=dropout)
...
class AttentionFeedback(nn.Module):
  ...
  def forward(self, embedded, conved, encoder_conved, encoder_combined, pred_conved, pred_combined):
    ...
    if self.multi_head:
      p_attented_encoding = self.multi_head_att(combined, pred_conved, pred_combined)
      p_attention = self.multi_head_att.attention.energy.sum(1).softmax(-1)
    else:
...
class Seq2SeqFeedback(nn.Module):
  ...
  def forward(self, enc_in, dec_in):
    ...
    pred_conved, pred_combined = self.p_encoder(output)
