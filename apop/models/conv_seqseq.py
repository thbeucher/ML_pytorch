...
class DecoderRelu(nn.Module):
  def __init__(self, output_dim, emb_dim, hid_dim, n_layers, kernel_size, dropout, pad_idx, device, embedder=None, max_seq_len=100,
    ...
    self.emb2hid = nn.Linear(emb_dim, hid_dim)
    self.decoders = nn.ModuleList([DecoderBlockRelu(hid_dim, emb_dim, kernel_size, pad_idx, dropout, device, score_fn=score_fn,
                                                    scaling_energy=scaling_energy, multi_head=multi_head, d_keys_values=d_keys_values)
                                                      for _ in range(n_layers)])
    self.hid2emb = nn.Linear(hid_dim, emb_dim)
...
class DecoderBlockRelu(nn.Module):
  def __init__(self, hid_dim, emb_dim, kernel_size, pad_idx, dropout, device, score_fn=F.softmax, scaling_energy=False,
    ...
    self.attention = AttentionRelu(hid_dim, emb_dim, device, score_fn=score_fn, scaling_energy=scaling_energy,
                                   multi_head=multi_head, d_keys_values=d_keys_values, dropout=dropout)
...
class DecoderFeedback(nn.Module):
  def __init__(self, output_dim, emb_dim, hid_dim, n_layers, kernel_size, dropout, pad_idx, device, embedder=None, max_seq_len=100,
    ...
    self.emb2hid = nn.Linear(emb_dim, hid_dim)
    self.decoders = nn.ModuleList([DecoderBlockFeedback(hid_dim, emb_dim, kernel_size, pad_idx, dropout, device, score_fn=score_fn,
                                                        scaling_energy=scaling_energy, multi_head=multi_head, d_keys_values=d_keys_values)
                                      for _ in range(n_layers)])
    self.hid2emb = nn.Linear(hid_dim, emb_dim)
...
class DecoderBlockFeedback(nn.Module):
  def __init__(self, hid_dim, emb_dim, kernel_size, pad_idx, dropout, device, score_fn=F.softmax, scaling_energy=False,
    ...
    self.attention = AttentionFeedback(hid_dim, emb_dim, device, score_fn=score_fn, scaling_energy=scaling_energy,
                                       multi_head=multi_head, d_keys_values=d_keys_values, dropout=dropout)
